...
def draw_bar_plot():
    ...
    df_bar['month'] = df_bar.index.strftime('%B')  # Full month name for proper legend

    # Group by year and month and calculate average page views
    df_bar_grouped = df_bar.groupby(['year', 'month'])['value'].mean().unstack()

    # Draw bar plot
    fig = df_bar_grouped.plot(kind='bar', figsize=(12, 6), legend=True)
    plt.title('Average Daily Page Views per Month', fontsize=16)
    plt.xlabel('Years', fontsize=12)
    plt.ylabel('Average Page Views', fontsize=12)
    plt.xticks(rotation=45)

    # Reorder legend labels to ensure months are displayed correctly
    plt.legend(title='Months', fontsize=10, loc='upper left', labels=[
